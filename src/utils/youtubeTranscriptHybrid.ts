...
export async function extractYouTubeContextHybrid(): Promise<VideoContext> {
  ...
  try {
    console.log('[Transcript] 🚀 Attempting fast InnerTube API extraction...');
    ...
    console.log(
      `[Transcript] ✅ Fast method succeeded in ${fastDuration}ms: ` +
    ...
    console.warn(
      `[Transcript] ⚠️ Fast method failed after ${fastDuration}ms:`,
      (fastError as Error).message
    );
    console.log('[Transcript] 🔄 Falling back to DOM scraping method...');
    ...
    try {
      ...
      console.log(
        `[Transcript] ✅ DOM fallback succeeded in ${fallbackDuration}ms ` +
      ...
      console.error(`[Transcript] ❌ Both methods failed after ${totalDuration}ms`);
